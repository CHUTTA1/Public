...
class Menu:
	...
	def cek_login(self,cookie):
		try:
			...
			prints(Panel(f"""{M2} WELCOME TO X CYBER""",width=80,style=f"{color_panel}"))
			exit()
...
class Crack:
		...
		try:
			for pw in pwx:
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					self.cp.append(user)
					tree = Tree("                        ")
					tree.add(f" X💔 {K2}{user}|{pw}{P2} ")
					#prints(tree)
					open(f"CP/{self.hari_ini}.txt","a").write(f"X CP🥲 | {user} | {pw}\n")
					break
	...
	def simpan_hasil(self):
		prints(f"""{K2}X CRACK OK DATE : MRX-OK.txt
{K2}X CRACK CP DATE : MRX-CP.txt\n""")
###----------[ MENU ]---------- ###
class Back:
